<?php
    ...
    function validate_name($name) {
        ...
        if (empty($name)) {
            array_push($result["errors"], array("field" => "name", "message" => "name is required")); 
            return false;
        }
        ...
        if (strpos($name, 'a') !== false) {
            array_push($result["errors"], array("field" => "name", "message" => "name already used"));
            return false;
        }
    ...
    function validate_email($email) {
        ...
        if (empty($email)) {
            array_push($result["errors"], array("field" => "email", "message" => "email is required"));
            return false;
        }
        
        // reject invalid email format
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            array_push($result["errors"], array("field" => "email", "message" => "invalid email format"));
            return false;   
        }
    ...
    function validate_comment($comment) {
        ...
        if (strlen($comment) > 50) {
            array_push($result["errors"], array("field" => "comment", "message" => "comment is too long"));
            return false;
        }
    ...
    function validate_website($website) {
        ...
        if (!empty($website)) {
            ...
            if (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i",$website)) {
                array_push($result["errors"], array("field" => "website", "message" => "invalid URL"));
                return false;
            }
        }
